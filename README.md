...
| `search.json` | The flat search surface, all plugins in one array. Rows are `kind: "node"` or `kind: "concept"`. Load only this to answer "which node or concept fits". |
...
| `<Plugin>.json` | Full node entries for one plugin, keyed by id, with the plugin's alias map and enum table. Load lazily when a search hit needs detail. |
| `schema/` | JSON Schemas for each of the above. |

## The id scheme

A node's id is its C++ settings class name, `UPCGExBevelPathSettings` for **Path : Bevel**. That choice is deliberate: the same name is what Unreal reflection reports for the installed plugin and what the engine's PCG tooling needs to place a node in a graph, so the id joins documentation, live editor state, and actuation without a mapping table.

**One resolution path.** To resolve any id: check the `nodes` map of each plugin file, then the `aliases` map. Every id the bundle emits resolves that way, wherever it appears:
